...
Citizen.CreateThread(function()
    while true do
        ...
        if not mailboxOpened and IsNearbyMailbox() then
			DrawText(_U("TextNearMailboxLocation"):gsub("%$1", Config.keyToOpen), 23, 0.5, 0.85, 0.50, 0.40, 255, 255, 255, 255)

            if IsControlJustReleased(0, Keys[Config.keyToOpen]) then
				OpenUI(false)
				Citizen.Wait(300)
            end
        end
